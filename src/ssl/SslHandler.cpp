...
int SslHandler::send(uint8_t* data, uint32_t length)
{
    if(NULL == ssl_) {
...
int SslHandler::send(iovec* iovs, uint32_t count)
{
    uint32_t bytes_sent = 0;
    for (uint32_t i=0; i < count; ++i) {
        int ret = SslHandler::send((uint8_t*)iovs[i].iov_base, iovs[i].iov_len);
        if(ret < 0) {
            return ret;
        } else {
            bytes_sent += ret;
...
int SslHandler::receive(uint8_t* data, uint32_t length)
{
    if(NULL == ssl_) {
    ...
    int ret = SSL_read(ssl_, data, length);
    int ssl_err = SSL_get_error(ssl_, ret);
    switch (ssl_err)
    {
        case SSL_ERROR_NONE:
