...
void WebSocketImpl::buildRequest()
{
    std::stringstream ss;
    ss << "GET  ";
    ss << uri_.getPath();
...
void WebSocketImpl::onReceive(int err)
{
    ...
    do {
        ...
        if(ret < 0) {
            ...
            if(getState() == STATE_OPEN) {
                if(!handleWsData((uint8_t*)buf, ret)) {
                    return;
                }
            } else if(getState() == STATE_RECVING_RESPONSE) {
                ...
                if(bytes_used < ret && getState() == STATE_OPEN) {
                    if(!handleWsData((uint8_t*)(buf+bytes_used), ret - bytes_used)) {
                        return;
                    }
                }
            }
...
bool WebSocketImpl::handleWsData(uint8_t* data, uint32_t len)
{
    bool destroyed = false;
    KUMA_ASSERT(nullptr == destroy_flag_ptr_);
    destroy_flag_ptr_ = &destroyed;
    WSHandler::DecodeError err = ws_handler_.decodeFrame(data, len);
    if(destroyed) {
        return false;
    }
    destroy_flag_ptr_ = nullptr;
    ...
    if(err != WSHandler::DecodeError::WS_ERROR_NOERR ||
       err != WSHandler::DecodeError::WS_ERROR_NEED_MORE_DATA) {
        cleanup();
        setState(STATE_CLOSED);
...
void WebSocketImpl::onHttpData(const char* data, uint32_t len)
{
    KUMA_ERRXTRACE("onHttpData, len="<<len);
}

void WebSocketImpl::onHttpEvent(HttpParser::HttpEvent ev)
{
